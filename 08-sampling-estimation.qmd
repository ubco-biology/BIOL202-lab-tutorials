...
OK, so we see that the minimum and maximum values are `r min(gene.means.n20$mean_length)` (the "Min." value) and `r max(gene.means.n20$mean_length)` (the "Max." value). So this can inform our x-axis limits when constructing the histogram (note that you typically need to try out different maximum values for the y-axis limits).
