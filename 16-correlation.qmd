...
We'll use the `wolf` and `trick` datasets (discussed in examples 16.2 and 16.5 in the text, respectively).
...
Explore the data, first with `glimpse` to check its structure:

```{r corr_seedata1}
wolf %>% glimpse()
```

This tells us `wolf` has 24 rows (observations) and 2 numeric (`<dbl>`) columns: `inbreed_coef` and `n_pups`. To check for missing values, follow up with `summary`:

```{r corr_seedata1b}
wolf %>% summary()
```

Neither column shows an "NA's" line below its Max. value, which tells us there are no missing values in either variable. If there WERE missing values, be sure to report the correct sample size in your results!

Now let's explore the `trick` dataset the same way:
...
`trick` has 21 rows and two numeric columns, "years" and "impressiveness_score" (their values happen to be whole numbers). Again, no "NA's" line appears for either column, so there are no missing values. Reading example 16.5 from the text, we see that "impressiveness_score" is a form of ranking variable.
...
```{r fig-16-association-between-inbreeding-coefficient, fig.cap = "The association between inbreeding coefficient and number of surviving wolf pups (n = 24).", fig.width = 4.2, fig.height = 4}
...
```{r fig-16-association-between-inbreeding-coefficient-2, fig.cap = "The association between inbreeding coefficient and number of surviving wolf pups (n = 24). Values have been jittered slightly to improve legibility.", fig.width = 4.2, fig.height = 4}
...
```{r fig-16-association-between-inbreeding-coefficient-3, fig.cap = "The association between inbreeding coefficient and number of surviving wolf pups (n = 24). Values have been jittered slightly to improve legibility.", fig.width = 4.2, fig.height = 4}
...
```{r fig-16-scatterplot-impressiveness-written-accounts, fig.cap = "Scatterplot of the impressiveness of written accounts of the Indian rope trick by firsthand observers and the number of years elapsed between witnessing the event and writing the account (_n_ = 21). Values have jittered slightly to improve legibility.", fig.width = 4.2, fig.height = 4}
...
```{r fig-16-scatterplot-impressiveness-written-accounts-2, fig.cap = "Scatterplot of the impressiveness of written accounts of the Indian rope trick by firsthand observers and the number of years elapsed between witnessing the event and writing the account (_n_ = 21). Values have jittered slightly to improve legibility.", fig.width = 4.2, fig.height = 4}
