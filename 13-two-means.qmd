...
```{r fig-13-histogram-differences-antibody-production, fig.cap = "Histogram of the differences in antibody production rate before and after the testosterone treatment", fig.width = 4, fig.height = 3}
...
```{r fig-13-paired-plot-antibody-production, fig.width = 4, fig.height = 3, fig.cap = "Paired plot of antibody production rate before and after the testosterone treatment"}

blackbird %>%
  ggplot(aes(x = time, y = antibody)) +
  geom_point(shape = 1, size = 1.5) +
  geom_line(aes(group = blackbird), colour = "grey") +
  theme_bw()
```
...
```{r fig-13-normal-quantile-plot-differences, fig.width = 4, fig.height = 3, fig.cap = "Normal quantile plot of the differences in antibody production rate before and after the testosterone treatment (ln[mOD/min]) 10^-3."}
...
Note that in a raw CSV, categories in a variable like `dominant_eye` might arrive abbreviated — e.g. "l" and "r" for "left" and "right". In the `biol202` package version of `students`, this has already been relabelled to "Left" and "Right" for you.

We can use the `unique` function to check all unique values of a categorical variable:
...
If you ever needed to relabel abbreviated categories like this yourself, you'd use the `recode_factor` function from the `dplyr` package, in conjunction with the familiar `mutate` function used to create a new variable (though here we're just over-writing an existing variable):
...
We don't actually run that here, since the categories are already correctly labelled. Running `recode_factor` on data that's already "Left"/"Right" wouldn't produce an error — but it *would* silently reverse which category is treated as the reference level (the one that comes first), which could shift the order categories appear in later tables and figures.
...
We'll learn a [better way](90-tables.qmd#stats_nicetable) to get around this later.
...
```{r fig-13-violin-boxplot-heights-students, fig.width = 3, fig.height = 4, fig.cap = "Violin and boxplot of the heights of students with right (n = 106) and left (n = 48) dominant eyes.  Boxes delimit the first to third quartiles, bold lines represent the group medians, and whiskers extend to 1.5 times the IQR. Points beyond whiskers are extreme observations."}
...
```{r fig-13-normal-quantile-plots-height, fig.width = 5, fig.height = 3, fig.cap = "Normal quantile plots of height for students with right (n = 106) or left (n = 48) dominant eyes."}
