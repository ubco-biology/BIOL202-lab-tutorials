...
For this tutorial we'll use the `circadian` dataset.  These data are associated with Example 15.1 in the text.
...
The `treatment` variable already arrives as a "factor" variable, and already in the correct order ("control", "knee", "eyes") — the `biol202` package took care of that. But the category labels themselves aren't capitalized, so let's fix that using the `recode_factor` function from the `dplyr` package, the method we previously learned in the [2-sample _t_-test tutorial](13-two-means.qmd#twosamp_ttest).

Be sure to provide the categories in the order you want them to appear in any tables, figures, and analyses — here, that order is already correct, so we're just relabelling each one:
...
The category names are now capitalized, and — as expected, since it was already correct — the ordering of the factor "levels" remains consistent with how it's displayed in the text figure in example 15.1.
...
```{r fig-15-stripchart-phase-shifts-circadian, fig.cap = "Stripchart of phase shifts in the circadian rhythm of melatonin production in 22 participants of an experiment. Solid circles denote group means, and bars +/- one standard error", fig.width = 4, fig.height = 4.5}
...
And present it:
```{r tbl-15-anova-table-circadian-rythm,}
...
```{r fig-15-stripchart-phase-shifts-circadian-2, fig.cap = "Stripchart of phase shifts in the circadian rhythm of melatonin production in 22 participants of an experiment. Solid circles denote group means, and bars +/- one standard error", fig.width = 4, fig.height = 4.5}
...
```{r fig-15-stripchart-showing-phase-shift, fig.cap = "Stripchart showing the phase shift in the circadian rhythm of melatonin production in 22 experimental participants given alternative light treatments. Solid circles represent group means, and bars represent +/- one SE. Group means sharing the same letter are not significantly different according to the Tukey-Kramer post-hoc test (family-wise $\\alpha$ = 0.05).", fig.width = 4, fig.height = 4.5}
