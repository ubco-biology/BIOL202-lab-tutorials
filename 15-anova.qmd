...
Let's recode this using the method we previously learned in the [2-sample _t_-test tutorial](13-two-means.qmd#twosamp_ttest), using the `recode_factor` function from the `dplyr` package (loaded as part of the `tidyverse`).
...
circadian.stats <- circadian %>%
  group_by(treatment) %>%
  summarise(
    Count = n() - naniar::sum(is.na(shift),
    Count_NA = naniar::sum(is.na(shift), 
