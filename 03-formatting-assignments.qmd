...
The next step is to get an overview of the dataset.  To do this, use the `glimpse` function from the `dplyr` package, which is already loaded.

```{r overview_data, eval = TRUE}
glimpse(students)
```

This shows we have 154 rows (observations) and 6 columns (variables) total. 
...
Once you're happy with the PDF you created, it's time to submit **your PDF document** (NOT your RMD file) to the appropriate place in Canvas.   
