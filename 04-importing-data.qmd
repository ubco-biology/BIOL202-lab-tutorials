...
This happens if you forgot to load the `readr` package! (See also [Common errors and their solutions](99-common-issues.qmd).)
:::
...
As we learned in the "Preparing and formatting assignments" [tutorial](03-formatting-assignments.qmd#example_answer), the `glimpse` function from the `dplyr` package (already loaded) gives you a compact overview of a data object.

Let's get an overview of the `trout` dataset, using the pipe approach:
...
* "Rows:" gives the number of observations, and "Columns:" gives the number of variables. So for `trout`, there are 9 observations and 3 variables.
* Each line below that describes one column (variable): its name, its type (in angle brackets), and then a preview of its actual values.
* The type abbreviation tells you whether a variable is categorical or numeric. Here, `site` is `<chr>` ("character" — text, i.e. categorical), while `day` and `trout_caught` are both `<dbl>` ("double precision" — a numeric type that allows decimals). In other datasets you may instead see `<fct>` ("factor" — R's other type for a categorical variable, one with a known, fixed set of categories, discussed in the box above) or `<int>` ("integer" — whole-number numeric data).

So `glimpse` already answers the first four questions listed above: there are 3 variables and 9 observations, `site` is categorical, and `day` and `trout_caught` are numeric.

That leaves the last question: are there any missing observations? For this, and for numeric summaries like the mean and quantiles, use the `summary` function. It's part of "base R" (R's built-in functionality), so no extra package is needed.
...
* For each numeric variable (`day` and `trout_caught`), `summary` reports six descriptive statistics: the minimum, the 1st quartile, the median, the <a href="https://ubco-biology.github.io/Procedures-and-Guidelines/glossary#Mean">mean</a>, the 3rd quartile, and the maximum.
* If a numeric variable had any missing observations, an extra line would appear below the maximum, reading something like "NA's :2", telling you how many values are missing. No such line appears for `day` or `trout_caught`, which tells us neither variable has any missing observations.
* The `site` variable is categorical, but since it's stored as plain text (type `<chr>`, not `<fct>`), `summary` doesn't count how many observations fall in each category — it just reports its length, class, and mode. When a categorical variable is instead stored as a factor (as most categorical variables in this course's datasets are), `summary` reports a count of observations in each category instead, along with an "NA's" line if any observations are missing. You'll see an example of this shortly, with the `penguins` dataset.
