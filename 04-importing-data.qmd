...
```{r exceltidy, fig.cap = "Tidy data in Excel", echo = FALSE, fig.width = 3}
...
If you're curious about data wrangling, the `dplyr` package, which is loaded with the `tidyverse` package, provides all the tools for data wrangling. Its associated cheatsheet is available at the package's [vignette website](https://dplyr.tidyverse.org/index.html). 
...
<!-- TODO(maintainer): this chunk is eval: false ONLY because the GitHub repo
     hosting csv_data/ does not exist yet. Once it is created and pushed,
     update the URL below and delete the `eval: false` lines in this chunk and
     the next one, so students see real output. See also _quarto.yml repo-url. -->
...
```{r readcsv_noshow_real}
#| echo: false
# Keeps the rest of the chapter working while the URL chunks above are
# switched off. Delete once the repo exists and those chunks run.
library(biol202)
data(students)
```
