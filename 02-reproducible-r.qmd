...
* Create a "_README.txt file for the root BIOL202 directory, as per instructions in the [Biology Procedures and Guidelines webpage](https://ubco-biology.github.io/Procedures-and-Guidelines/readme-files-and-data-dictionaries.html). The [UBC Library](https://ubc-library-rc.github.io/rdm/content/03_create_readme.html) has an excellent guide on README files too. 
...
* Within the BIOL202 root directory, create three (sub)directories: 
  ...
  - "BIOL202_tutorials"
  - "BIOL202_assignments"
* Create a "_README.txt" file in each of the three sub-directories (again, you can use RStudio to create these!), OR ensure the _README.txt file in the root directory explains the contents of all subdirectories.  

An example setup is illustrated below. Note it is optional to have a README file in each directory... you can instead make the root README more extensive.

```{r fig-02-example-directory-structure, fig.cap = "Example directory structure", echo = FALSE, fig.width = 3}
...
Included in the `biol202` package is the `here` package, which helps with file and directory management and navigation. 
...
::: {.callout-note}
Every time you run `library(here)`, you'll see a message like this:

```
here() starts at /Users/yourname/OneDrive/BIOL202/BIOL202_tutorials
```

This is not an error — it's `here` confirming which folder it has identified as your project's home base (the "working directory"), which is what lets it find your files. You'll see this message every time you load the package, in every session.
:::
...
```{r down_rmd, eval = F}
download.file("https://raw.githubusercontent.com/ubco-biology/BIOL202/refs/heads/main/more/BIOL202_tutorial_markdown.Rmd",destfile= "BIOL202_tutorial_markdown.Rmd")
```
