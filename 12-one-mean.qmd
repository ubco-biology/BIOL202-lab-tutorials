...
```{r fig-12-frequency-body-temperature, fig.cap = 'Frequency distribution of body temperature (degrees Farenheit) for 25 randomly chosen healthy people.', fig.width = 4, fig.height = 3}
...
```{r fig-12-t-distribution-df-24, echo = FALSE, warning = FALSE, message = FALSE, fig.cap = "The *t* distribution for df = 24.", fig.width = 4, fig.height = 3.5}
tval <- round(qt(alpha/2, df = d_f, lower.tail = FALSE), 2)
linedata <- data.frame(x1 = tval, x2 = tval, y1 = 0, y2 = 0.3)
# Labels are plain strings here, rendered as maths by `parse = TRUE` in the
# annotate() calls below. Passing expression() or bquote() objects directly to
# annotate() no longer works in ggplot2 4.0.
tcrit_textpos <- "italic(t)[crit]"
tcrit_textneg <- "italic(-t)[crit]"
ggplot(data.frame(x = c(-4, 4)), aes(x)) +
  ...
  annotate("text", x = 3, y = 0.03, label = "alpha/2", parse = TRUE) +
  annotate("text", x = -3, y = 0.03, label = "alpha/2", parse = TRUE) +
  annotate("text", x = 2.2, y = 0.32, label = tcrit_textpos, parse = TRUE) +
  annotate("text", x = -2.2, y = 0.32, label = tcrit_textneg, parse = TRUE) +
  theme_bw()
```
...
```{r fig-12-example-inline-markdown-r, fig.cap = "Example of inline markdown and R code for a confidence interval", echo = FALSE, fig.width = 5}
