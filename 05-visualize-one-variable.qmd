...
* the `tigerdeaths` dataset contains data associated with example 2.2A in the Whitlock and Schluter text
* the `birds` dataset contains counts of different categories of bird observed at a marsh habitat
...
We'll get an overview of the data using the `glimpse` function, as we learned in the [Preparing and Importing Tidy Data](04-importing-data.qmd#data_overview) tutorial:
...
In a [previous tutorial](04-importing-data.qmd#data_overview) you learned the important information to look for when getting an overview of a dataset using the `glimpse` and `summary` functions.

::: {.callout-note}
**TIP**
It's important to check whether there are any missing values for any of the variables in your dataset. `glimpse` doesn't report missing-value counts, so use `summary` for that:
...
There are 344 cases (rows) in `penguins`, but you'll see an "NA's :2" line beneath the Min./Max. values for each of the four morphometric variables — `bill_length_mm`, `bill_depth_mm`, `flipper_length_mm`, and `body_mass_g` — telling you that each of them is missing 2 observations. (`sex` is missing 11 observations too, though it isn't a morphometric measurement.) **You need to take note of this so that you report the correct sample sizes in any table or figure captions!**
:::
...
**Load and data overview:** Following the instructions provided in previous tutorials, load the `tigerdeaths` and `birds` datasets with `data()`, and get an overview of each of those datasets.
:::
...
We'll demonstrate this  using the `tigerdeaths` dataset that you should have loaded as part of a suggested activity in the previous section, using code like this:

::: {.callout-note}
**TIP**
Some datasets that we use in these tutorials need to be loaded into your workspace with `data()` before you can use them.  This is the case with the `tigerdeaths` dataset, and the code for loading it is below. Other datasets, like the `penguins` dataset, exist within packages (palmerpenguins), and their objects are already created for you the moment you load that package. Most of the time, and particularly for your lab assignments, you need to explicitly load a package dataset with `data()`, as we do below.
:::

```{r vis1_read_tiger, message = FALSE}
# here we load the "tigerdeaths" dataset from the biol202 package
data(tigerdeaths)
```

You would also have gotten [overview of the data](#vis_data_overview) as part of the activity, using the `glimpse` function.  This would have shown that the `activity` variable is of type `<fct>` ("factor" — R's type for a categorical variable with a fixed set of categories) — which tells us it is a **categorical** variable — and that there are 88 cases (rows) in the dataset. You'll see exactly how many categories `activity` has in a moment, when we build the frequency table below.
...
```{r tbl-05-frequency-table-showing-activities, echo = FALSE}
...
```{r fig-05-bar-graph-showing-activities, fig.cap = "Bar graph showing the activities of 88 people at the time they were attached and killed by tigers near Chitwan national Park, Nepal, from 1979 to 2006", fig.width = 6, fig.height = 6}
...
```{r fig-05-example-code-chunk-producing, fig.cap = "Example code chunk for producing a good bar graph", echo = FALSE, fig.width = 5}
...
```{r fig-05-histogram-body-mass-g, fig.cap = "Histogram of body mass (g) for 342 penguins", fig.width = 6, warning = FALSE}
...
* We use the `geom_histogram` function, which has its own optional arguments:
    - the "colour" we want the outlines of each bar in the histogram to be
...
::: {.callout-note}
You may also see a message like `Removed 2 rows containing non-finite values`. This is `ggplot2` telling you that some observations were left out of the graph because their value was missing (`NA`) — in this case, the 2 penguins with no recorded body mass. It is not an error; it is a useful reminder to double-check your sample size when you write your figure caption.
:::
...
```{r fig-05-histogram-body-mass-g-2, fig.cap = "Histogram of body mass (g) for 342 penguins", fig.width = 6, warning = FALSE}
...
```{r fig-05-histogram-body-mass-g-3, fig.cap = "Histogram of body mass (g) for 342 penguins", fig.width = 6, warning = FALSE}
